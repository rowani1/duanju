...
从 v0.2.0 起，你可以用自己的知识库目录**按文件覆盖**内置 `knowledge/`：加载每个知识文件时，先查你的目录里有没有**同构相对路径**的同名文件，有就用你的版本，没有就自动回落内置版。只想改一个文件就只放一个文件，CLI 升级后新增的内置文件也会自动生效。
...
1. **命令行参数**（临时用一次，相对路径相对当前目录）：
   ...
   `package` / `title` / `write` / `revise` / `run` 五个调模型的命令都支持 `--knowledge`；`run` 会透传给全部子步骤。
